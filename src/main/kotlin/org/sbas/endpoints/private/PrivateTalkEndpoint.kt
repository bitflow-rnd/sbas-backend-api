...
class PrivateTalkEndpoint {
    ...
    @Operation(summary = "대화방 생성", description = "대화방 생성 API")
    @POST
    @Path("chat-room")
    fun regChatRoom(regRequest: RegTalkRoomDto): Response {
        return Response.ok(talkService.regChatRoom(regRequest)).build()
    }
