...
class InfoPtRepository : PanacheRepositoryBase<InfoPt, String> {
  ...
  fun findInfoPtList(param: InfoPtSearchParam): List<InfoPtSearchDto> {
    ...
    return entityManager.createQuery(query, InfoPtSearchDto::class.java).setMaxResults(15)
  ...
  fun findByIds(ptId: List<String?>): List<InfoPt> {
    return list("where ptId in ?1", ptId)
  }
...
class InfoInstRepository : PanacheRepositoryBase<InfoInst, String> {
  ...
  fun findFireStatn(instId: String): InfoInst {
    return find("instTypeCd = 'ORGN0002' and id = '$instId'").firstResult()
      ?: throw NotFoundException("fire station not found")
  }
...
class InfoCntcRepository : PanacheRepositoryBase<InfoCntc, InfoCntcId> {
  ...
  fun getHistSeq(userId: String): Int? =
    find("from InfoCntc where id.userId = '$userId' order by id.histSeq desc").firstResult()?.id?.histSeq

  fun getMyUsers(userId: String) = find("from InfoCntc where id.userId = '$userId'").list()

  fun findInfoCntcByUserIdAndMbrId(userId: String, mbrId: String) =
    find("from InfoCntc where id.userId = '$userId' and id.mbrId = '$mbrId'").firstResult()
