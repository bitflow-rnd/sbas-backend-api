...
class BdasReqRepository : PanacheRepositoryBase<BdasReq, BdasReqId> {
  ...
  fun findByPtIdWithLatestBdasSeq(ptId: String) =
    find("from BdasReq where id.ptId='$ptId' order by id.bdasSeq desc").firstResult()
  ...
  fun queryForBdasList(cond: String?, offset: Int?): TypedQuery<BdasListDto> {
    val query =
      "select new org.sbas.dtos.bdas.BdasListDto(br.id.ptId, br.id.bdasSeq, pt.ptNm, pt.gndr, fn_get_age(pt.rrno1, pt.rrno2), " +
